...
<li><strong>responseType {OPTIONAL, string}</strong>: Response type for all authentication requests. Defaults to <code>'token'</code>. It can be any space separated list of the values <code>code</code>, <code>token</code>, <code>id_token</code>. <strong>If you don't provide a global <code>responseType</code>, you will have to provide a <code>responseType</code> for each method that you use</strong></li>
<li><strong>responseMode {OPTIONAL, string}</strong>: The default responseMode used, defaults to <code>'fragment'</code>. The <code>parseHash</code> method can be used to parse authentication responses using fragment response mode. Supported values are <code>query</code>, <code>fragment</code> and <code>form_post</code>. The <code>query</code> value is only supported when <code>responseType</code> is <code>code</code>.</li>
...
<pre class="prettyprint source lang-js"><code>auth0.parseHash({ hash: window.location.hash }, function(err, authResult) {
  if (err) {
...
<blockquote>
<p><strong>Important:</strong> If you're not using the hosted login page to do social logins, you have to use your own <a href="https://manage.auth0.com/#/connections/social">social connection keys</a>. If you use Auth0's dev keys, you'll always get <code>login_required</code> as an error when calling <code>checkSession</code>.</p>
</blockquote>
<p>Remember to add the URL where the authorization request originates from, to the Allowed Web Origins list of your Auth0 client in the <a href="https://manage.auth0.com/">Dashboard</a> under your client's <strong>Settings</strong>.</p>
<ul>
